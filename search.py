...
def depthFirstSearch(problem):
    ...
    while (True):
        if stack.isEmpty():
            return []
        pos, frontier = stack.pop()
        visited.append(pos)
        if problem.isGoalState(pos):
            return frontier

        successor = problem.getSuccessors(pos)
        if successor:
            for item in successor:
                if item[0] not in visited:
                    new_path = frontier + [item[1]]
                    stack.push((item[0], new_path))
        ...
        if successor:
            for item in successor:
                if item[0] not in visited and (item[0] not in (state[2][0] for state in queue.heap)):
                    ...
                    queue.push((item[0], new_path), priority)
                    ...
                    new_priority = problem.getCostOfActions(frontier + [item[1]])

                    if old_priority > new_priority:
                        new_path = frontier + [item[1]]
                        queue.update((item[0], new_path), new_priority)
